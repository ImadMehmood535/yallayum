...
const Page = () => {
  ...
  const getData = async () => {
    try {
      const response = await API.getMyCustomOrders();
            ...
            <div className="p-6">
              {item?.orderItems?.map((product, productIndex) => (
                ...
                >
                  <div className="md:w-1/3">
                    <Image
                      ...
                      width={300}
                      height={300}
                      className="rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 max-h-[150px] max-w-[80px]"
                    />
                  </div>
                ...
                <span className="text-3xl font-bold text-black">
                  AED {parseFloat(item?.deliveryCharges).toFixed(2)}
                </span>
              </div>
              <div className="bg-gray-50 mt-4 px-6 py-4 flex justify-between items-center rounded-lg">
                <span className="text-lg font-semibold text-gray-700">
                  Total Price: {item?.totalItems}
                </span>
                <span className="text-3xl font-bold text-black">
                  AED{" "}
                  {(parseFloat(item?.totalPrice) + parseFloat(item?.deliveryCharges)).toFixed(2)}
                </span>
              </div>
            </div>
          )}
